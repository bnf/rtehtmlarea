			...
			items: [new Toolbar({
				...
				new Iframe({
					...
					autoEl: {
						...
						tag: 'iframe',
						cls: 'editorIframe',
						src: UserAgent.isGecko ? 'javascript:void(0);' : (UserAgent.isWebKit && !UserAgent.isChrome ? 'javascript: \'' + Util.htmlEncode(this.config.documentType + this.config.blankDocument) + '\'' : HTMLArea.editorUrl + 'Resources/Public/Html/blank.html')
					},
					isNested: this.isNested,
					nestedParentElements: this.nestedParentElements,
					editorId: this.editorId
